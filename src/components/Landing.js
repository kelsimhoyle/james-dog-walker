...
const Trail = ({ children, open }) => {
  ...
  const trail = useTrail(items.length, {
    config: { mass: 10, tension: 900, friction: 200 },
    opacity: open ? 1 : 0,
    x: open ? 0 : 40,
    height: open ? 75 : 0,
          ...
          <div className="relative md:z-10 pb-8 bg-white sm:pb-16 md:pb-20 lg:max-w-2xl lg:w-full lg:pb-28 xl:pb-32">
            ...
            <main className=" mx-auto max-w-7xl px-4  sm:px-6">
              <div className="sm:text-center lg:text-left">
                <h1 className="text-4xl tracking-tight font-extrabold text-gray-900  md:text-6xl pt-10 h-48 md:h-56">
                    <Trail open={true}>
                    <span>{title}</span>
                    <span className="text-green-500 ">{subtitle}</span>
                    </Trail>
                </h1>
                  <p className="mt-3 text-base text-gray-500 sm:mt-5 text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0 h-24">
                    {paragraph}
                  </p>
